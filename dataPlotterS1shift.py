...
for s1x in s1xList:
    for s1y in s1yList:
        ...
        ax_s1x.append(np.ones(np.size(wavelength))*s1x)
        ax_s1y.append(np.ones(np.size(wavelength))*s1y)
        ax_wavelength.append(wavelength)
        ax_s21.append(s21_2)
        
        ax_s1x2.append(np.ones(np.size(w2))*s1x)
        ax_s1y2.append(np.ones(np.size(w2))*s1y)
        ax_w2.append(w2)
        ax_ng.append(ng)

ax_s1x = np.array(flattenList(ax_s1x,samplingNum=5))
ax_s1y = np.array(flattenList(ax_s1y,samplingNum=5))
ax_wavelength = np.array(flattenList(ax_wavelength,samplingNum=5))
ax_s21 = np.array(flattenList(ax_s21,samplingNum=5))

ax_s1x2 = np.array(flattenList(ax_s1x2,samplingNum=5))
ax_s1y2 = np.array(flattenList(ax_s1y2,samplingNum=5))
...
sc1 = ax1.scatter(ax_s1x,ax_s1y,ax_wavelength,s=10*(-ax_s21-(0))/(np.max(-ax_s21)-(0)),c=ax_s21,cmap='jet',vmin=-50)
fig.colorbar(sc1,ax=ax1)

ax2 = fig.add_subplot(122,projection='3d')
sc2 = ax2.scatter(ax_s1x2,ax_s1y2,ax_w2,s=10*(ax_ng/np.max(ax_ng)),c=ax_ng,cmap='jet',vmin=0)
